...
eval $(kubectl get nodes \
  ...
  grep -v $INTERNAL_IP |
  xargs -n1 printf "bridge fdb append to 00:00:00:00:00:00 dst %s dev net0;")
...
eval $(kubectl get nodes \
  --output=jsonpath='{range .items[*]}{.status.addresses[?(@.type=="InternalIP")].address} {.spec.podCIDR} {"\n"}{end}' | 
  grep -v $INTERNAL_IP | 
  awk '{GW=$2;gsub("10.200","172.16",GW); gsub("0/24","1",GW); printf "ip route add " $2 " via " GW "; "}')
....
...
.Grab the ip for one of the worker nodes
....
BASTION=$(triton ip kubernetes)
...
....
ssh root@$BASTION curl http://${IP}:${NODE_PORT}
....
